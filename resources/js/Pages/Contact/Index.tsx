        ...
        <PublicLayout>
            ...
            <div className="bg-[#F8FDF9] py-20 border-b border-green-50">
                <div className="container mx-auto px-4 text-center">
                     ...
                     <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">Jangan Ragu Menghubungi Kami</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid gap-6">
                            {[
                                { icon: MapPin, title: 'Alamat Sekolah', value: schoolProfile?.address || 'Alamat belum diatur', color: 'bg-red-50 text-red-600' },
                                { icon: Phone, title: 'Telepon / WhatsApp', value: schoolProfile?.phone || '-', color: 'bg-green-50 text-green-600' },
                                { icon: Mail, title: 'Email Resmi', value: schoolProfile?.email || '-', color: 'bg-blue-50 text-blue-600' },
                                { icon: Clock, title: 'Jam Operasional', value: schoolProfile?.operating_hours || 'Senin - Jumat: 07:00 - 16:00', color: 'bg-amber-50 text-amber-600' },
                            ].map((item, i) => (
                    ...
                    <div className="bg-white p-8 md:p-10 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100">
                         <div className="mb-8">
                            ...
                            <p className="text-gray-500">Isi formulir di bawah ini, kami akan segera merespon pesan Anda.</p>
                        </div>
                        ...
                        <form onSubmit={submit} className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <Input id="name" value={data.name} onChange={e => setData('name', e.target.value)} required placeholder="Contoh: Budi Santoso" className="h-12 bg-gray-50 border-gray-200 focus:bg-white" />
                                {errors.name && <p className="text-red-500 text-sm">{errors.name}</p>}
                            </div>
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <Label htmlFor="email" className="text-base">Email</Label>
                                    <Input id="email" type="email" value={data.email} onChange={e => setData('email', e.target.value)} required placeholder="nama@email.com" className="h-12 bg-gray-50 border-gray-200 focus:bg-white" />
                                    {errors.email && <p className="text-red-500 text-sm">{errors.email}</p>}
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="phone" className="text-base">No. WhatsApp</Label>
                                    <Input id="phone" value={data.phone} onChange={e => setData('phone', e.target.value)} placeholder="0812..." className="h-12 bg-gray-50 border-gray-200 focus:bg-white"/>
                                    {errors.phone && <p className="text-red-500 text-sm">{errors.phone}</p>}
                                </div>
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="message" className="text-base">Pesan</Label>
                                <Textarea id="message" value={data.message} onChange={e => setData('message', e.target.value)} rows={6} required placeholder="Tuliskan pesan Anda disini..." className="bg-gray-50 border-gray-200 focus:bg-white resize-none" />
