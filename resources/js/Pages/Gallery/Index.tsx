        ...
        <PublicLayout>
            ...
            <div className="bg-[#F8FDF9] py-20 border-b border-green-50">
                <div className="container mx-auto px-4 text-center">
                    <Badge className="bg-white text-primary border-primary/20 mb-4 px-4 py-1">Portofolio Siswa</Badge>
                    <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">Karya & Inovasi Siswa</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                     {projects.data?.length > 0 ? projects.data.map((px: any) => (
                        <div key={px.id} className="group h-full">
                            <Card className="border-0 shadow-lg hover:shadow-2xl transition-all duration-300 h-full flex flex-col overflow-hidden rounded-2xl bg-white border-t-4 border-t-transparent hover:border-t-primary">
                                <div className="aspect-video bg-gray-100 relative overflow-hidden">
                                    ...
                                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold shadow-sm text-primary uppercase tracking-wide">
                                        Project
                                    </div>
                                </div>
                                <CardContent className="p-8 flex-1 flex flex-col">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-6">
                                         {px.technologies?.split(',').map((tech: string, i: number) => (
                                             <span key={i} className="px-3 py-1 bg-green-50 text-emerald-700 text-xs font-medium rounded-md border border-green-100">
                                                {tech.trim()}
                                             </span>
                                         ))}
                                    </div>
                        ...
                        <div className="col-span-full py-20 text-center">
                            ...
                            <h3 className="text-2xl font-bold text-gray-900 mb-2">Belum ada karya ditampilkan</h3>
                            <p className="text-gray-500">Nantikan update karya terbaik siswa kami.</p>
                        </div>
                     )}
                </div>
                </div>
